...
//fonction qui interprete le pseudo code binaire
void translateByteCode(uint8_t instruction, uint8_t operand) {
    switch(instruction) {
...
int main() {
    ...
    while(instruction!=Instructions::END_CODE) {
        memory.lecture(currentAddress++, &instruction);
        _delay_ms(10);
        memoire.lecture(currentAddress++, &operand);
        translateByteCode(instruction, operand);
    }
